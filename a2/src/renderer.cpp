...

{
	...
	pass1Prog->activate();
	// bind the shadow gbuffer to gbTextureUnit 0
	// Is also the Texture unit 2 on GPU

	gbuffer->BindTexture(SHADOW_GBUFFER, 0);

	//Basiclly what here done is to turn the fragment shader output into shadow buffer (gBuffer)
	...
	//IMG1 goes here
	//The ortho is the P in MVP
	//The Lookat(where looking from, where looking to, up dir for cam) is sort of MV in MVP, the world to view transformation
	//The up dir for cam is perpendicular to lookat dir
	...
	//Clear the buffer that associate with current framebuffer object
	// This the shadow buffer
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	glEnable(GL_DEPTH_TEST);

	//Calculate object to light clipping coordinate system
	...
	gbuffer->BindTexture(SHADOW_GBUFFER, 0); //setup shadow buffer , allow shader lookup particular texture unit
	pass2Prog->setInt("shadowBuffer", BASE_GBUFFER_TEXTURE_UNIT + 0); // gbTextureUnit 0 //will be the integer corresponed to texture unit that stores the shadowbuffer, as identifier
