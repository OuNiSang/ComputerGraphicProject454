...

{
  if (action == GLFW_PRESS)
    ...
    case 'M':
      ...
      cout << (scene->showPhotonMap ? "show" : "hide") << " photon map" << endl;
      redisplay = true;
      break;
    ...
    case '/':
      cout
	<< endl
	<< "b     build photon map if not built; otherwise clear photon map" << endl
	<< "m     show/hide photon map" << endl
...

{
  ...
  if (action == GLFW_PRESS) {    // Handle mouse press
    ...
    if (button == GLFW_MOUSE_BUTTON_RIGHT || (button == GLFW_MOUSE_BUTTON_LEFT && (keyModifiers & GLFW_MOD_SHIFT))) { // 'arcball' handles right mouse event
