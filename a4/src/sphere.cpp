...

{
  ...
  if (t0 < 0.001 && t1 < 0.001)
    return false; // both intersections are behind rayStart (0.001 is just barely off the surface in direction 'rayDir')

  // Pick farther point if rayStart is on the surface of the sphere
...
// Find farther point on sphere from viewPos in viewDir

float Sphere::farthestDist( vec3 viewPos, vec3 viewDir )

{
  return (centre - viewPos) * viewDir.normalize() + 1.0; // 1.0 = radius
}
