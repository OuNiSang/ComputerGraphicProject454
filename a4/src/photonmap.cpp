...

{
	...
	if (rnd < Pd) //Is in Diffuse Reflection
	{
		...
		forwardTraceRay(intPoint, diffuseDir, a * power, objIndex, objPartIndex, scene, specularDone, depth + 1);//Note: a multiply can be removed
		specularDone = false;
	}
	else if (rnd < Pd + Ps) // Is in Specular Reflection, and no power lost on specular?
	{
		...
		specularDone = true;
		forwardTraceRay(intPoint, R, a * power, objIndex, objPartIndex, scene, true, depth + 1); //Note: a multiply can be removed
	}
