...
class Object {
 ...
 public:
  ...
  virtual float farthestDist( vec3 viewPos, vec3 viewDir ) { return -MAXFLOAT; }

  virtual void input( istream &istream ) {}
  virtual void output( ostream &ostream ) const {}

  void computeMaxDistFromEye( vec3 viewPos, vec3 viewDir ) {
    maxDistFromEye = farthestDist( viewPos, viewDir );
  }
