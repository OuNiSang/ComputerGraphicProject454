...

{
 ...
 // Create the FBO
  ...
  for (int i = 0 ; i < numTextures; i++) {
    ...
    glTexImage2D( GL_TEXTURE_2D, 0, textureTypes[i], fbWidth, fbHeight, 0, GL_RGB, GL_FLOAT, NULL );
...

{
  ...
  for (int r=0; r<dim; r++)
	...
	drawStrokeString( string(buffer), c/(float)dim*2-0.95, (dim-r-1)/(float)dim*2-0.95, 0.03, 0.0 );
